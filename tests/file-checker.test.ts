...
result = add("hello", "world")  # Type error
  ...
  test("should format diagnostics correctly", () => {
    const result = {
      ...
      tool: "tsc",
      diagnostics: [
        { line: 10, column: 5, severity: "error" as const, message: "Type 'number' is not assignable to type 'string'" },
        { line: 20, column: 15, severity: "warning" as const, message: "Variable is declared but never used" }
      ]
    };
